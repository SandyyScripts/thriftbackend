...
router.post("/create-checkout-session", async (req, res) => {
  try {
    ...
    if (orderId) {
      ...
      if (orderData.shippingAddressSnapshot && orderData.shippingAddressSnapshot.name) {
        compressedData.address = {
          name: orderData.shippingAddressSnapshot.name,
          email: orderData.shippingAddressSnapshot.email,
          phone: orderData.shippingAddressSnapshot.phone,
          street: orderData.shippingAddressSnapshot.street,
          city: orderData.shippingAddressSnapshot.city,
          state: orderData.shippingAddressSnapshot.state,
          zip: orderData.shippingAddressSnapshot.zipCode,
          country: orderData.shippingAddressSnapshot.country,
        };
      }
...
router.post("/verify-payment-status", async (req, res) => {
  try {
    ...
    if (order.paymentStatus === "PENDING" && order.updatedAt < oneHourAgo) {
      ...
      if (orderSession) {
        if (orderSession.payment_status === "COMPLETED") {
    ...
    if (event.type === "checkout.session.completed") {
      ...
      if (orderId) {
        ...
        if (shippingDetails || customerDetails) {
          updateData.shippingAddressSnapshot = shippingDetails || customerDetails;
        ...
        if (existingOrder.paymentStatus !== "COMPLETED") {
          ...
          if (orderWithItems) {
            ...
            for (const item of orderWithItems.orderItems) {
              ...
              try {
                if (item.variantId) {
                  console.log(`   → ProductVariant detected, updating stock...`);
                  const variantBefore = await prisma.productVariant.findUnique({
                    where: { id: item.variantId },
                    select: { id: true, name: true, stock: true }
                  });
                  console.log(`   → Variant BEFORE: ${JSON.stringify(variantBefore)}`);
                  ...
                  if (variantBefore) {
                    ...
                    console.log(`   ✓ Variant AFTER: ${JSON.stringify(updatedVariant)}`);
                  }
        ...
        if (!updatedOrder.shipmentId) {
          try {
            ...
            const shippingAddress = updatedOrder.shippingAddressSnapshot as any;
            if (shippingAddress) {
        ...
        try {
          ...
          if (customerEmail) {
            ...
            if (orderWithItems) {
              const shippingAddr = updatedOrder.shippingAddressSnapshot as any;
        ...
        try {
          ...
          try {
            ...
            console.log("🔍 Getting shipping rates for address:", {
              name: orderData.shippingAddressSnapshot.name,
              street1: orderData.shippingAddressSnapshot.street1,
              city: orderData.shippingAddressSnapshot.city,
              state: orderData.shippingAddressSnapshot.state,
              zip: orderData.shippingAddressSnapshot.zip,
              country: orderData.shippingAddressSnapshot.country,
            });
            ...
            if (preSelectedRate && preSelectedRate.objectId) {
              ...
              const rates = await getShippingRates(
                orderData.shippingAddressSnapshot as any,
                [{
                  length: '6',
                  width: '4',
                  height: '2',
            ...
            if (selectedRate) {
              const shipmentResult = await createShipment({
                orderId: newOrder.id,
                toAddress: orderData.shippingAddressSnapshot as any,
                parcels: [{
                  length: '6',
                  width: '4',
                  height: '2',
          ...
          try {
            ...
            await sendOrderConfirmationEmail(customerEmail, {
              orderId: newOrder.id,
              customerName: orderData.shippingAddressSnapshot.name || 'Customer',
              total: newOrder.total,
